...
router.post(
  ...
  [
    ...
    [
      check(
        "pin",
        "You are required to set a four-digit transaction Pin"
      ).isNumeric().isLength({ min: 4, max: 4 }),
    ],
  ],
  async (req, res) => {
    ...
    try {
      ...
      if (wallet) {
        ...
        res.status(400).json({errors:[{msg:"You already have an active wallet"}]})
      }
      wallet = new Wallet({
        user: req.user.id,
      });
...
router.post('/deposit', [auth, [
 check('amount','Enter a valid amount').isNumeric()
]], async(req,res)=>{
 const error = validationResult(req)
 if(!error.isEmpty()){
  return res.status(400).json({errors: error.array() })
 ...
 try {
  ...
  wallet.save()
  res.json({msg:`${amount} has been deposited into your wallet`, newTransaction})
 } catch (err) {
  console.error(err.message)
  res.status(500).send("Server Error")  
 }
...
router.post('/transfer',[auth, [
 ...
 check('pin', "Enter a valid 4 digit pin").isNumeric().isLength({min:4,max:4}),
 check('bank', 'Please enter beneficiary bank').not().isEmpty(),
 check('acc_num','Please enter valid beneficiary account number').isNumeric().isLength({min:10,max:10})
]], async(req, res)=>{
 const error = validationResult(req)
 if(!error.isEmpty()){
  return res.status(400).json({errors: error.array() })
 ...
 try {
  ...
  wallet.transactions.push(newTransaction)
  wallet.save()
  res.json({msg:`You have successfully sent N${amount}`, newTransaction})
 } catch (err) {
  console.error(err.message)
  res.status(500).send("Server Error")  
 }
